...
public:
	...
	//获取整个buffer
	char* get_buffer(){return m_base;}
	//获取总数据大小
	uint32_t get_size(){return m_size;}

	//从有效数据后面获取一个大小为size的buffer用于添加数据
	char* get_append_buffer(uint32_t size);
	//设置添加到buffer中的大小
	void set_append_size(uint32_t size);

	//返回可获取的数据大小
	uint32_t get_fetch_size();
	//获取有效数据的buffer用于获取数据,返回有效数据的size;没有数据返回NULL,size设置为0
	char* get_fetch_buffer(uint32_t *size=NULL);
	//设置从buffer中取出的数据大小;
	void set_fetch_size(uint32_t size);
public:
	//添加大小为size的bytes数组到buffer后面
	bool append_bytes(char *bytes, uint32_t size);
	//添加字符串(不包含'\0')到buffer后面
	bool append_str(char *str);
	//添加32位整数i到buffer后面
	...
	bool fetch_bytes(char *buf, uint32_t size);
	//严格从buffer读出32位整数.数据不足size字节返回false.
	bool fetch_int32(int32_t *i);
	//严格从buffer读取64位整数.数据不足size字节返回false.
	bool fetch_int64(int64_t *i);
private:
	char *m_base;
	uint32_t m_capacity;
	uint32_t m_size;
