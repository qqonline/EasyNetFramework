...
ERROR_CODE TransHandler::OnEventRead(int32_t fd, uint64_t now_time)
{
	...
	else
	{
		...
		LOG_DEBUG(logger, "continue to receive data packet. data_type="<<context->type<<", expect_size="<<context->header_size+context->body_size<<", cur_size="<<context->Size<<", fd="<<fd);
	}

	if(context->type == DTYPE_INVALID)  //协议类型未知,需要解码协议头来判断是文本协议还是二进制协议
	{
		if(context->Capacity < context->header_size)
		{
			bool result = context->Enlarge(context->header_size);
			assert(result == true);
		}
		char *buffer         = context->Buffer+context->Size;
		uint32_t buffer_size = context->Capacity-context->Size;
		uint32_t need_size   = context->header_size-context->Size;
		int32_t recv_size    = ReadData(fd, buffer, buffer_size, need_size);
		if(recv_size < 0)
		{
			if(recv_size == -1)
				LOG_ERROR(logger, "read data error. expect_size="<<context->header_size<<", cur_size="<<context->Size<<", fd="<<fd);
			else
		...
		DecodeResult result = protocol_factory->DecodeHeader(context->Buffer, context->type, context->body_size);
		if(result == DECODE_ERROR)
		{
			LOG_ERROR(logger, "decode header error. expect_size="<<context->header_size<<", cur_size="<<context->Size<<", fd="<<fd);
			m_RecvFdMap.erase(it);
			m_AppInterface->DeleteProtocolContext(context);
			return ECODE_ERROR;
		}
		...
		{
			LOG_DEBUG(logger, "wait for more header data. expect_size="<<context->header_size<<", cur_size="<<context->Size<<", fd="<<fd);
			return ECODE_PENDING;
		}

		//解码头部成功
		assert(context->type != DTYPE_INVALID);
		LOG_DEBUG(logger, "decode header succ. data_type="<<context->type<<", header_size="<<context->header_size<<", body_size="<<context->body_size<<", cur_size="<<context->Size<<", fd="<<fd);
	}

	//读可能剩下的二进制协议体数据
	//if(context->type==DTYPE_BIN && context->bytebuffer->Size<context->header_size+context->body_size)
	if(context->Size<context->header_size+context->body_size)
	{
		if(context->Capacity < context->header_size+context->body_size)
		{
		...
		char *buffer         = context->Buffer+context->Size;
		uint32_t buffer_size = context->Capacity-context->Size;
		uint32_t need_size   = context->header_size+context->body_size-context->Size;
		int32_t recv_size    = ReadData(fd, buffer, buffer_size, need_size);
		if(recv_size < 0)
		{
			if(recv_size == -1)
	...
	if(result == DECODE_ERROR)
	{
		LOG_ERROR(logger, "decode body error. data_type="<<context->type<<", header_size="<<context->header_size<<", body_size="<<context->body_size<<", cur_size="<<context->Size<<", fd="<<fd);
		m_RecvFdMap.erase(it);
		m_AppInterface->DeleteProtocolContext(context);
		return ECODE_ERROR;
	}
	...
	{
		LOG_DEBUG(logger, "wait for more body data. data_type="<<context->type<<", header_size="<<context->header_size<<", body_size="<<context->body_size<<", cur_size="<<context->Size<<", fd="<<fd);
		return ECODE_PENDING;
	}
...
ERROR_CODE TransHandler::OnEventWrite(int32_t fd, uint64_t now_time)
{
	...
	while(true)
	{
		...
		//发送数据
		char *buffer = context->Buffer+context->send_size;
		uint32_t need_size = context->Size-context->send_size;
		int32_t send_size = Socket::Send(fd, buffer, need_size);
		if(send_size == -1)    //socket有问题,发送失败,返回false
		{
			LOG_ERROR(logger, "send data error. context="<<context<<", data_type="<<context->type<<", total_size="<<context->Size<<", send_size="<<context->send_size<<", fd="<<fd);
		...
		if(context->send_size >= context->Size)  //发送完成
		{
			LOG_DEBUG(logger, "send data finished. context="<<context<<", data_type="<<context->type<<", total_size="<<context->Size<<", send_size="<<context->send_size<<", fd="<<fd);
			if(it != m_SendFdMap.end())
			{
				m_SendFdMap.erase(it);
				it = m_SendFdMap.end();
		...
		//只发送了一半数据,等待下次可写事件
		LOG_INFO(logger, "send data partly. context="<<context<<", data_type="<<context->type<<", total_size="<<context->Size<<", send_size="<<context->send_size<<", fd="<<fd);
