...
#define TYPE_I64   1
#define TYPE_BYTES 2
#define TYPE_BP0   2    //byte,填充0字节
#define TYPE_BP1   3    //byte,填充1字节
#define TYPE_BP2   4    //byte,填充2字节
#define TYPE_BP3   5    //byte,填充3字节
...
{
	uint32_t len = sizeof(uint16_t)*2+size;
	if(type == TYPE_BYTES)
	{
		len += sizeof(uint32_t);       //for size;
		type += len%sizeof(uint32_t);
		len += len%sizeof(uint32_t);   //考虑地址对齐
	}
	...
	*(uint16_t*)ptr = type;
	...
	memcpy((void*)ptr, (void*)bytes, size);
